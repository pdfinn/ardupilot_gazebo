#!/bin/bash
#
# NERVA2 UAS Demo - SITL Launcher
# Based on start_sitl_gazebo.sh but configured for edge demo
#
# Differences from parent start_sitl_gazebo.sh:
#   - Sends MAVLink to Android device (same as original)
#   - Video stream is handled by separate start_demo_video.sh
#   - Does NOT modify any parent directory files
#
...
echo -e "${GREEN}════════════════════════════════════════${NC}"
echo -e "${GREEN}  NERVA2 UAS Demo - SITL Launcher${NC}"
echo -e "${GREEN}════════════════════════════════════════${NC}"
echo ""
...
# This logic is preserved exactly from start_sitl_gazebo.sh
if [ -z "$ANDROID_IP" ]; then
    ...
    if [ "$DEVICE_COUNT" -eq 0 ]; then
        ...
        echo "The demo requires an Android device with ATAK + UAS Tool."
...
echo "  MAVLink → Android ($ANDROID_IP:$ANDROID_PORT)"
echo "  ATAK UAS Tool → CoT → TAK Server (hephaestus)"
echo "  NERVA2 → uasmcp → TAK Connector → TAK Server"
echo ""
